...
// JQuery meta links
$jq_google         = '<script src="//ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js"></script>'; 
$jq_ui             = '<script src="//ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/jquery-ui.min.js"></script>';
$jq_ui_themes      = '<link rel="stylesheet" href="//ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/themes/smoothness/jquery-ui.css" />';
$jq_ui_accordion   = '<link rel="stylesheet" href="_css/jq-ui-accordion.css" type="text/css" />';
...
// TABLE NAMES
$symposium2013TBL  = "apps_symp2013";
...
$feedingForumPage   = "http://www.nomasinternational.org/_smf/";
...
global $trainingDatesTBL;
  ...
  $link = db_connect_site();   		  
  if($result = mysqli_query($link, "SELECT * FROM $trainingDatesTBL WHERE year = '$thisYear' LIMIT 1")) {	
     $answer = mysqli_num_rows($result);
	 mysqli_free_result($result);	  
  }
  return $answer;
...
global $trainingSitesTBL, $feedingForumPage, $trainingDatesTBL, $thisYear, $today, $contactPage, $testingPage;
   ...
   if($result_s = mysqli_query($link, "SELECT * FROM $trainingSitesTBL WHERE visible AND loc_id IN (SELECT loc_id from $trainingDatesTBL where starts >= '$today')")) {	
      if (mysqli_num_rows($result_s) > '0' ) {
         while($row = mysqli_fetch_assoc($result_s)) {
			...
			echo "<div class='text'><span class='ital'><a href='" . $url . "' target='_blank'>" . $location . "</a></span></div>\n";
