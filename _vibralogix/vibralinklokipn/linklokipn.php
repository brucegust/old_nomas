<?php
// **************************************************************************************************
// Linklok Paypal V2.6 digital download verification
// Copyright (c) 2003-2013 Vibralogix
// www.vibralogix.com
...
error_log("\n\n".'IPN Link Accessed: '.date("F j, Y, g:i a"), 3, dirname(__FILE__)."/../../error_log");

// Uncomment to use sandbox mode to test a 1 cent transaction
//if ( $_POST['mc_gross'] == '0.01' ){ $UseSandbox = 1; }

// SECURITY, SETUP AND ADMIN VARS
$LinklokURL      = "http://www.nomasinternational.org/_vibralogix/vibralinklokipn/linklokipn.php"; // URL of the linlok.php page on your site
$PDTtoken        = "NILtcazsz-F6T4XJRb4ImoVXPX8EJr-aHlSabCnX8C7MNiicvL6QJyZbqsi"; // Auto Return PDT token from paypal-profile-website payment preferences
$LinkKey         = "DiN#l1h-Z53@7_29x";                  // Encryption key for download links
$PaypalEmail     = "marjorie@nomasinternational.org";    // PRIMARY Paypal email address
$SellerCompany   = "Therapeutic Media";                  // Your company name
$SellerEmail     = "marjorie@nomasinternational.org";    // Your email address for order enquiries
$SellerURL       = "http://www.nomasinternational.org";  // Your website URL
$CopyEmail       = "";                                   // Email to receive copy of order emails
$ManualPassword  = "poeb";                               // Password for manual order entry. "" to disable
$DelayEchecks    = "N";                                  // Set to Y to delay eCheck orders until cleared.

// DATABASE AND TABLES
$DbHostLinklok         = "localhost";                    // db Host
$DbNameLinklok         = "nomasint_punkysite";           // db Name
$DbUserLinklok         = "nomasint_s0r3n";               // db User
$DbPasswordLinklok     = "Edzp0DyR7H";                   // db User Password
$DbTableNameOrders     = "ceu_orders";                   // Tbl for orders
$DbTableNameProducts   = "ceu_products";                 // Tbl for products
$DbTableNameUsers      = "ceu_users";                    // Tbl for users - (custom add CODE AT LINE 5296-ish)

// TABLE PRODUCT VARS
$DbProductsId          = "id";
$DbProductsDescription = "description";
$DbProductsPrice       = "price"; 
$DbProductsLocation    = "location";
$DbProductsExpiry      = "expiry";
$DbProductsExtra       = "extra";
$DefaultLocation       = "../../olceu/olceu_pdf/";

// TEMPLATES AND MISC
$EmailTemplate = "tmpl_purchase_thanks.html";  // Optional Email Template in either .txt or .html format
$DownloadTemplate = "tmpl_download_done.html"; // return from purchase
$ErrorTemplate = "tmpl_error.html";            // Optional Error page template
...
$DownloadBackground="tmpl_download_bkg.html";  // Displays while files and downloading
$Txnid = "mysql";                              // Optional to stop possible multiple calls from IPN
...
if ($ProductCSV != "")
{
  ...
  while (!feof($fh))
  {
    ...
    if ((strlen($lne) > 2) && (substr($lne, 0, 4) != "<!--") && (substr($lne, 0, 2) != "//"))
    {
      ...
      // If field 3 is numeric then we should assume $DefaultCurrency (or USD) as the currency
...
{
  $mysql_linklok=llipn_dbconnect($DbHostLinklok,$DbUserLinklok,$DbPasswordLinklok);
  if ($mysql_linklok==0)
  {
...
{
  $mysql_result = llipn_mysql_query("SELECT count(*) from ".$DbTableNameProducts,$mysql_linklok);
  if ($mysql_result!==false) 
  { 
    $row = llipn_mysql_fetch_row($mysql_result);
    if ($row!=false)
      $numberofproducts = $row[0];
  }  
  else
    $numberofproducts=0;
}
...
if (($ManualPassword != "") && (isset($password)) && ($ManualPassword!=$password) && ($createbuttoncode==1))
{
  ShowMessage($ErrorTemplate, "Incorrect password.", "");
  exit;
}

// If password entered and createbuttoncode then display buttons
if ((isset($createbuttoncode)) && ($ManualPassword != "") && ($ManualPassword == $password))
{
  if ((!isset($DbTableNameProducts)) || ($DbTableNameProducts==""))
  {
    // For products stored in the script or external product file
    for ($k=0;$k<$numberofproducts;$k++)
    {
      $id=trim(strtok($Products[$k], $ProductDelimiter));
      $description =trim(strtok($ProductDelimiter));
      $description = str_replace("^", "", $description);
      $costs=explode(":", trim(strtok($ProductDelimiter)));
      $currency=trim(strtok($costs[0],"="));
      $price=trim(strtok("="));
      if ($price=="")
      {
        $price=$currency;
      ...
      ?>
      <span style='color: black; font-weight: bold; font-size: 12pt; font-family: arial;'><?php echo "$description ($id)   $currency $price"; ?></span><br>
      ...
      <input type="image" src="https://www.paypal.com/en_US/i/btn/x-click-but10.gif" alt="Add to cart">
      </form>
      </textarea>
      <br>      
      <span style='color: black; font-weight: normal; font-size: 10pt; font-family: arial;'>If you prefer a simple link instead of a button then <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_cart&add=1&business=<?php echo urlencode($PaypalEmail); ?>&item_number=<?php echo $id; ?>&item_name=<?php echo urlencode($description); ?>&amount=<?php echo $price; ?>&currency_code=<?php echo $currency;?>">right click and copy here</a></span><br>
      <br>      
      <br> 
  ...
  {
    // For products stored in a DB
    $query="SELECT * FROM ".$DbTableNameProducts;
    $mysql_result=llipn_mysql_query($query,$mysql_linklok);
    $found = 0;
    while($row=llipn_mysql_fetch_array_assoc($mysql_result))
    {
      $id=trim($row[$DbProductsId]);
      $description=trim($row[$DbProductsDescription]);
      $costs=explode(":",trim($row[$DbProductsPrice]));
      $currency=trim(strtok($costs[0],"="));
      $price=trim(strtok("="));
      if ($price=="")
      {
      ...
      <span style='color: black; font-weight: bold; font-size: 12pt; font-family: arial;'><?php echo "$description ($id)   $currency $price"; ?></span><br>
      <br>      
      <span style='color: black; font-weight: normal; font-size: 10pt; font-family: arial;'><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_cart&add=1&business=<?php echo urlencode($PaypalEmail); ?>&item_number=<?php echo $id; ?>&item_name=<?php echo urlencode($description); ?>&amount=<?php echo $price; ?>&currency_code=<?php echo $currency;?>">Right click and copy for link</a></span><br>
      <br>      
      <textarea cols="100" rows="11">
      <form target="paypal" action="https://www.paypal.com/cgi-bin/webscr" method="post">
      <input type="hidden" name="add" value="1">
...
// If ?orderform, testform or addtocartlinks then request password
if ((isset($orderform)) || (isset($ORDERFORM)) || (isset($testform)) || (isset($TESTFORM)) || (isset($createbuttoncode)))
{
  print "<html><head>\n";
  print "<meta http-equiv=\"content-type\" content=\"text/html; charset=".$llcharset."\">\n";
  print "<title>Linklok Paypal</title></head><body>\n";
  ...
  if (isset($createbuttoncode))
    print "<input type=\"hidden\" name=\"createbuttoncode\" value=\"1\">\n";
  print "<p align=\"left\"><font face=\"Arial\" color=\"#333399\"><span style=\"font-size:16pt;\"><b>Linklok Paypal</b></span></font></p>\n";
...
if (($ManualPassword != "") && ($ManualPassword == $password) && ($testtype==""))
{
  ...
  if ($testmode!="Y")  
  {
    ...
    {
      print "<input type=\"hidden\" name=\"manualipaddr\" value=\"".trim(strtok($_SERVER['REMOTE_ADDR'],","))."\">\n";
      print "<input type=\"hidden\" name=\"manualiplevel\" value=\"-1\">\n";
    }
  }
  else
  {
  ...
  print "<font face=\"Arial\"><span style=\"font-size:10pt;\"><input type=\"text\" name=\"last_name\" size=\"39\"></span></font></p></td></tr>\n";
...
{
  error_log("\n".'IPN:  Processing manual order', 3, dirname(__FILE__)."/../../error_log");
  ...
  else
  {
    if ($ManualEmailTemplate!=""){
      ...
      error_log("\n".'IPN: Manual order, $EmailTemplate = $ManualEmailTemplate', 3, dirname(__FILE__)."/../../error_log");
    }
    $mc_gross="0.00";  
  }  
}
...
if (isset($llauth))
{
  ...
  // See if redirect download required
  if (!isset($redirectall))
    $redirectall=0;
  $pos = strpos(strtoupper($extra), "REDIRECT=1");
  if ((is_integer($pos)) || ($redirectall==1))
  {
  ...
  if (is_integer($pos))
  {
    ...
    if ($NotifyDownloadEmail != "")
      EmailDownloadNotify($fname, $item_number, $desc, $txn_id, trim(strtok($_SERVER['REMOTE_ADDR'],",")), $expiry, $email);
...
//    $req .= "&sig=" . $_GET['sig'];
  ...
  { 
    if (strcmp (trim($lines[$i]), "SUCCESS") == 0)
    {
      $starti=$i;
      break;
    }  
  ...
  if ($starti>-1)
  {
    for ($i=$starti; $i<count($lines);$i++)
    {
      list($key,$val) = explode("=", $lines[$i]);
      $keyarray[urldecode($key)] = urldecode(rtrim($val));
    }      
...
else if ($calledasautoreturn != 1)
{
  if ($VerifyCurl==1)
  {
    if ($UseSandbox==1)
    {
      ...
      $curlport=443;
      error_log("\n".'IPN: $UseSandbox is 1', 3, dirname(__FILE__)."/../../error_log");
    }  
    else
    {
    ...
    curl_setopt($ch, CURLOPT_PORT, $curlport);     
    curl_setopt($ch, CURLOPT_HEADER, 0);
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
...
if ($res == "VERIFIED")
{
  error_log("\n".'IPN: Verified', 3, dirname(__FILE__)."/../../error_log");
  if ($DbTableNameOrders != ""){
    storeordermysql();
    error_log("\n".'IPN: Store order in MySQL DB', 3, dirname(__FILE__)."/../../error_log");
  }
} 
else {
  error_log("\n".'IPN: Not verified. $res='.$res, 3, dirname(__FILE__)."/../../error_log");
}
...
if ($res != "VERIFIED")
{
  ...
  error_log("\n".'IPN:'."Transaction not authenticated. payment_status=" . $payment_status . " pending_reason=" . $pending_reason . " result=" . $res, 3, dirname(__FILE__)."/../../error_log");
...
if ($calledasreturn == 0)
{
  error_log("\n".'IPN: Called as return 0', 3, dirname(__FILE__)."/../../error_log");
...
if ($calledasreturn == 1)
{
  error_log("\n".'IPN: Called as return 1. "If referrer details known then email details.."', 3, dirname(__FILE__)."/../../error_log");
  ...
  if ($cookiefound)
  {
	  ...
	  if ($DbTableNameOrders!="")
	  {
      $Query = "UPDATE " . $DbTableNameOrders . " SET referrerfirst=".llipn_quote_smart($reflinklokvals[4]).", entryfirst=".llipn_quote_smart($reflinklokvals[3]);
      $Query.=", referrernow=".llipn_quote_smart($sesreflinklokvals[4]).", entrynow=".llipn_quote_smart($sesreflinklokvals[3])." WHERE orderno=" . llipn_quote_smart($txn_id);
      $mysql_result = llipn_mysql_query($Query, $mysql_linklok);
	  }
  }
}
...
if ($Txnid != "")
{
  ...
  {
    error_log("\n".'IPN: "Order number '.$txn_id.' has already been processed"', 3, dirname(__FILE__)."/../../error_log");
...
{
  if (!isset($UseSandbox) || $UseSandbox != 1)
  {
    error_log("\n".'IPN: "Paypal email address does not match"', 3, dirname(__FILE__)."/../../error_log");
...
if ($manualorder == false)
{
  if ($nocurrency == true)
  {
    error_log("\n".'IPN: "Payment made with a currency that is not accepted"', 3, dirname(__FILE__)."/../../error_log");
...
// Compare price of product(s) in database vs. the price paid via PayPal.
if ($manualorder == false)
{  
  // Only check if we're not in sandbox mode
  if (!isset($UseSandbox) || $UseSandbox != 1)
  {
    ...
    if (isset($discount))
    {
      ...
      {
        error_log("\n".'IPN: "Amount paid is not sufficient for products ordered(1)"', 3, dirname(__FILE__)."/../../error_log");
    ...
    else
    {
      ...
      {
        error_log("\n".'IPN: "Amount paid is not sufficient for products ordered(2): "'.$totalpaid.' | '.$totalordered, 3, dirname(__FILE__)."/../../error_log");
        ShowMessage($ErrorTemplate, "Amount paid is not sufficient for products ordered.", $ErrorEmail);
        exit;
      }
    }
...
if ($manualorder == false)
{
  ...
  {
    error_log("\n".'IPN: "Button appears to have been tampered with."', 3, dirname(__FILE__)."/../../error_log");
...
if ($DbTableNameOrders != "")
{
  ...
  //error_log("\n".'IPN: did storeorderllprodsmysql()', 3, dirname(__FILE__)."/../../error_log");
}
...
if (function_exists('linklokorders_createorder'))
{
  ...
  if (($DbHostLinklok!="") && ($DbUserLinklok!="") && ($DbPasswordLinklok!="") && ($DbNameLinklok!=""))
  {
    ...
    {
      ShowMessage($ErrorTemplate, "Error connecting to MySQL", $ErrorEmail);
      error_log('IPN: Error connecting to MySQL', 3, dirname(__FILE__)."/../../error_log");
      exit;
    }
  }            
}      
...
if ($calledasreturn == 0)
{
  error_log("\n".'IPN: Called as return 0, again', 3, dirname(__FILE__)."/../../error_log");

  // Send either custom or predefined download links email
  if ((count($ProdId) > 0) || ($AlwaysEmail==true))
  {
    error_log("\n".'IPN: Send either custom or predefined download links email', 3, dirname(__FILE__)."/../../error_log");
    ...
    if (($tempcount == count($ProdId)) && ($tempcount > 0)){
      error_log("\n".'IPN: $EmailTemplate = $newtemplate', 3, dirname(__FILE__)."/../../error_log");
      $EmailTemplate = $newtemplate;
    }

    error_log("\n".'IPN: calling SendEmail', 3, dirname(__FILE__)."/../../error_log");
...
{
  error_log("\n".'IPN: SendEmail is running', 3, dirname(__FILE__)."/../../error_log");

  global $SellerCompany, $SellerEmail, $SellerURL, $HTMLEmail, $manualorder,$llcharset;
  if ($Template == "NONE")
    return;
  if ($Template!="")
  {  
  ...
  else
  {
    ...
    if ($HTMLEmail=="Y")
    {
      ...
      $mailBody.="<html>\n";
      $mailBody.="<head>\n";
      $mailBody.="<meta http-equiv=\"content-type\" content=\"text/html; charset=".$llcharset."\">\n";      
      ...
      $mailBody.="<!--unknownend-->\n";
      $mailBody.="If you have any questions regarding this order please contact us.<br>\n";
      $mailBody.="<br>\n";
    ...
    else
    {
      ...
      $mailBody.="<!--unknownend-->\n";
      $mailBody.="If you have any questions regarding this order please contact us.\n";
      $mailBody.="\n";
...
function EmailTemplateProcess($mailBody, $ProdId, $ProdDesc, $ProdLink, $ProdSize, $ProdExp, $ProdFile, $ProdExtra, $nd, $ext)
{
  ...
  global $DownloadBackground, $llcharset,$txn_id;
  ...
  error_log('IPN: SendEmailOut to '.$payer_email.' with subject: '.$subject, 3, dirname(__FILE__)."/../../error_log");

  // SEND COPY OF EMAIL
  if ($CopyEmail != "")
  {
    $subject = "A copy of Linklok Paypal email was sent to $payer_email";
...
function DisplayPage($Template, $ProdId, $ProdDesc, $ProdLink, $ProdSize, $ProdExp, $ProdFile, $ProdExtra, $nd)
{
  ...
  global $SellerCompany, $SellerEmail, $SellerURL, $HTMLEmail, $llcharset;
  if ($Template!="")
  {
    $ext = fileextension($Template);  
    if ($ext == ".php")
  ...
  else
  {
    ...
    $page.="<html>\n";
    $page.="<head>\n";
    $page.="<meta http-equiv=\"content-type\" content=\"text/html; charset=".$llcharset."\">\n";
    ...
    $page.="<!--unknownend-->\n";
    $page.="<br>\n";
    $page.="If you have any questions regarding this order please contact us.<br>\n";
    $page.="<br>\n";
...
{
  error_log("\n".'IPN: Function SendNotifyEmail()', 3, dirname(__FILE__)."/../../error_log");
...
function InsertVariables(&$buf)
{
  ...
  global $mc_shipping, $shipping_method,$discount,$contact_phone;
  // Now output affiliate code if not manual order
  ...
  if (isset($discount))
    $buf = str_replace("!!!discount!!!", $discount, $buf);
  else
    $buf = str_replace("!!!discount!!!", "0.00", $buf);    
  $buf = str_replace("!!!telephone!!!", $contact_phone, $buf);
 }
...
function InsertLoopVariables(&$buffer, $ProdId, $ProdDesc, $ProdLink, $ProdSize, $ProdExp, $ProdFile, $ProdExtra, $temptype, $ext)
{
  ...
  global $SitelokUseNameOnly,$SitelokCustom,$RandomPasswordMask;
  ...
  {  
    ...
    if ($exists == 1)
      ...
      $sitelokpassword=sl_CreatePassword($RandomPasswordMask);
    // If mysql being used reconnect as Sitelok may have closed connection
    if (($DbHostLinklok!="") && ($DbUserLinklok!="") && ($DbPasswordLinklok!="") && ($DbNameLinklok!=""))
    {
      $mysql_linklok=llipn_dbconnect($DbHostLinklok,$DbUserLinklok,$DbPasswordLinklok);
  ...
  if (count($ProdId)>0)
  {
    if ($DbTableNameOrders!="")
    {
      $query="SELECT * FROM ".$DbTableNameOrders." WHERE orderno=".llipn_quote_smart(trim($txn_id));
      $mysql_result=llipn_mysql_query($query,$mysql_linklok);
      if ($row=llipn_mysql_fetch_array_assoc($mysql_result))
      {
        $d = $row['serialnumber'];
  ...
  // If this product already handled then make sure details stored in $loitemdataarray are not altered
  $tmploitemdata="";
  if ($loitemdataarray[0]!="")
    $tmploitemdata=$loitemdataarray[0];
  if (count($ProdId)>0)
  ...
  if ($tmploitemdata!="")
    $loitemdataarray[0]=$tmploitemdata;  
  if (function_exists('linklokorders_createorder'))
  {
  ...
  if (count($sitelokgroupsarray) > 0)
  {
    ...
    // Handle any custom field variables
	//what's commented out doesn't work with PHP 7
	/*
    for ($k=1;$k<=50;$k++)
    {
	...
	*/
    ...
    if ($DbTableNameOrders!="")
    {
      $Query = "UPDATE " . $DbTableNameOrders . " SET sitelokusername=".llipn_quote_smart($sitelokusername).", sitelokpassword=".llipn_quote_smart($sitelokpassword)." WHERE orderno=" . llipn_quote_smart($txn_id);
      $mysql_result = llipn_mysql_query($Query, $mysql_linklok); 
    }
  } 
  ...
  global $SitelokHideExistingPassword;
  if ((isset($SitelokHideExistingPassword)) && ($SitelokHideExistingPassword!=""))
    $buffer = str_replace("!!!sitelokpassword!!!", $SitelokHideExistingPassword, $buffer);                    
  else
    $buffer = str_replace("!!!sitelokpassword!!!", $sitelokpassword, $buffer);                    
  // Store any serial numbers etc
  if (($DbTableNameOrders!="") && (count($ProdId)>0))
  {
    $Query = "UPDATE " . $DbTableNameOrders . " SET serialnumber=".llipn_quote_smart(implode($sernoarray,"|")).", softwarepassportname=".llipn_quote_smart(implode($spnamearray,"|")).", softwarepassportkey=".llipn_quote_smart(implode($spkeyarray,"|")).", userfunction=".llipn_quote_smart(implode($userfuncarray,"|")).", clicklocker=".llipn_quote_smart(implode($clicklockerarray,"|"))." WHERE orderno=" . llipn_quote_smart($txn_id);
    $mysql_result = llipn_mysql_query($Query, $mysql_linklok); 
  }  
...
function ProcessProductVariables(&$repeatbuf, $k, $ProdId, $ProdDesc, $ProdLink, $ProdSize, $ProdExp, $ProdFile, $ProdExtra, $temptype, $ext, &$sitelokusername, &$sitelokpassword, &$sitelokgroupsarray, &$sitelokexpiresarray,&$sernoarray,&$spnamearray,&$spkeyarray,&$userfuncarray,&$clicklockerarray)
{
  ...
  if ($sitelokpassword!="")
  {
    global $SitelokHideExistingPassword;
    if ((isset($SitelokHideExistingPassword)) && ($SitelokHideExistingPassword!=""))
      $repeatbuf = str_replace("!!!sitelokpassword!!!", $SitelokHideExistingPassword, $repeatbuf);
    else
...
function GetProductDetails($id, $currency, &$cost, &$description, &$url, &$size, &$expiry, &$extra)
{
  ...
  else
  {
    ...
    // Match against item id (item_number)
    $query="SELECT * FROM ".$DbTableNameProducts." WHERE ".$DbProductsId."=".llipn_quote_smart(trim($id));
    $mysql_result=llipn_mysql_query($query,$mysql_linklok);
    if ($row=llipn_mysql_fetch_array_assoc($mysql_result))
    {
      $found = 1;
...
function VerifyTransactionId($fn, $on)
{
  ...
  if (strtolower($fn) == "mysql")  
  {
    ...
    $query="SELECT * FROM ".$DbTableNameOrders." WHERE orderno=".llipn_quote_smart(trim($orderno));
    $mysql_result=llipn_mysql_query($query,$mysql_linklok);
    if ($row=llipn_mysql_fetch_array_assoc($mysql_result))
    {
    ...
    if ($calltype=="return")
      $Query = "UPDATE " . $DbTableNameOrders . " SET `return`='Y' WHERE orderno=" . llipn_quote_smart(trim($orderno));
    else  
      $Query = "UPDATE " . $DbTableNameOrders . " SET `ipn`='Y' WHERE orderno=" . llipn_quote_smart(trim($orderno));
    $mysql_result = llipn_mysql_query($Query, $mysql_linklok);
    return(1);
  }    
...
function filesize_remote($url, $timeout = 3)
{
...
function llipn_quote_smart($value)
{    
  if (function_exists("mysqli_real_escape_string"))
  {
    ...
    $value = "'" . mysqli_real_escape_string($mysql_linklok,$value) . "'";
    return $value;
  }
  else
  {
...
function llipn_dbconnect()
{
  global $DbNameLinklok,$DbHostLinklok,$DbUserLinklok,$DbPasswordLinklok,$mysql_linklok;
  if (function_exists("mysqli_connect"))
    $mysql_linklok=mysqli_connect($DbHostLinklok,$DbUserLinklok,$DbPasswordLinklok);
  else  
  ...
  if (function_exists("mysqli_select_db"))
    $db=mysqli_select_db($mysql_linklok,$DbNameLinklok);
  else
    $db=mysql_select_db($DbNameLinklok,$mysql_linklok);    
  if ($db==false)
    return(false);
  return($mysql_linklok);
}

function llipn_mysql_query($query,$mysql_linklok)
{
  if (function_exists("mysqli_query"))
    return mysqli_query($mysql_linklok,$query);
  return(mysql_query($query,$mysql_linklok));
}

function llipn_mysql_fetch_array_assoc($mysql_result,$ignore=0)
{ 
...
function llipn_mysql_fetch_row($mysql_result)
{ 
...
function llipn_mysql_close($mysql_linklok)
{
  if (function_exists("mysqli_close"))
    return(mysqli_close($mysql_linklok));
  return(mysql_close($mysql_linklok));
}

function SendEmailOut($toemail, $fromemail, $fromname, $subject, $mailBody, $htmlformat)
{
  error_log('SendEmailOut from IPN:'."\n", 3, dirname(__FILE__)."/../../error_log");

  global $EmailHeaderNoSlashR, $ExtraMailParam, $ErrorTemplate, $ErrorEmail, $UsePHPmailer, $UsePearMail;
  global $Custom_Mail_Headers, $llcharset;
  ...
  if ($htmlformat=="Y")
  {
    $headers .= "Content-type: text/html; charset=".$llcharset."\r\n";
  ...
  // Loop through all recipient emails and send mail with PHP mail()
  for ($k=0; $k<count($sendtoemail); $k++)
  {
    if ($ExtraMailParam != ""){
    ...
    // Testing mail output to error log
    //error_log($sendtoemail[$k]."\n".$subject."\n".$mailBody."\n".$headers."\n\n", 3, dirname(__FILE__)."/../../error_log");
  }

}

function EmailDownloadNotify($fname, $in, $desc, $ordnum, $ipaddr, $expiry, $email, $invoice)
{
...
function ClickLocker($url,$name,$email)
{
  ...
  {
    return("Clickloker support requires Curl");
  }
 ...
 * =======================
 * Insert order in orders table and insert user + orderID in users table
 */
function storeordermysql()
{
  ...
  global $mysql_linklok, $DbTableNameOrders, $DbTableNameUsers, $discount;

  // Payment completed and not reversing transaction
  if (($payment_status == "Completed") && ($txn_type != "reversal"))
  {
    $itemsordered = "";
    $itemnamesordered="";
    ...
    $Query.=llipn_quote_smart($txn_id).",";
    $Query.=llipn_quote_smart($parent_txn_id).",";
    $Query.=llipn_quote_smart(date("Y-n-d H:i:s",time())).",";
    if ($manualorder)
      $Query.=llipn_quote_smart("MANUAL-PAYPAL").",";
    else 
      $Query.=llipn_quote_smart("PAYPAL").",";      
    $Query.=llipn_quote_smart($first_name." ".$last_name).",";
    $Query.=llipn_quote_smart($payer_business_name).",";
    $Query.=llipn_quote_smart($address_street).",";
    $Query.=llipn_quote_smart("").",";   // Steet 2 not used in Paypal
    $Query.=llipn_quote_smart($address_city).",";
    $Query.=llipn_quote_smart($address_state).",";
    $Query.=llipn_quote_smart($address_zip).",";
    $Query.=llipn_quote_smart($address_country).",";
    $Query.=llipn_quote_smart($payer_email).",";
    $Query.=llipn_quote_smart("").",";  // Telephone not used in Paypal
    $Query.=llipn_quote_smart("").",";  // Fax not used in Paypal
    $Query.=llipn_quote_smart("").",";  // shipping address not used in Paypal
    $Query.=llipn_quote_smart("").",";  // shipping address not used in Paypal
    $Query.=llipn_quote_smart("").",";  // shipping address not used in Paypal
    $Query.=llipn_quote_smart("").",";  // shipping address not used in Paypal
    $Query.=llipn_quote_smart("").",";  // shipping address not used in Paypal
    ...
    $Query.=llipn_quote_smart("").",";  // shipping address not used in Paypal
    $Query.=llipn_quote_smart("").",";  // shipping address not used in Paypal
    $Query.=llipn_quote_smart("").",";  // shipping telephone not used in Paypal
    $Query.=llipn_quote_smart($memo).",";
    $Query.=llipn_quote_smart($invoice).",";
    if ($clientipaddr!="0.0.0.0")
      $Query.=llipn_quote_smart($clientipaddr).",";
    else 
      $Query.=llipn_quote_smart("").",";
    $Query.=llipn_quote_smart("").",";
    $Query.=llipn_quote_smart("").",";
    $Query.=llipn_quote_smart("").",";
    $Query.=llipn_quote_smart("").",";      
    ...
    $Query.=llipn_quote_smart($mc_currency).",";
    if ($manualorder)  
      $Query.=llipn_quote_smart("MANUAL").",";
    else 
      $Query.=llipn_quote_smart($payment_type).",";      
    $Query.=llipn_quote_smart($option_selection1).",";
    $Query.=llipn_quote_smart($option_selection2).",";
    $Query.=llipn_quote_smart($custom).",";
    $Query.=llipn_quote_smart("").",";   // Custom 4 not used in Paypal
    $Query.=llipn_quote_smart($discount).",";
    $Query.=llipn_quote_smart("").",";  // voucher
    $Query.="0.00".",";  // voucher value not used in Paypal
    if ($num_cart_items>0)
      $Query.=$num_cart_items.",";
    else  
      $Query.="1,";
    $Query.=llipn_quote_smart($itemsordered).",";
    $Query.=llipn_quote_smart($itemnamesordered).",";
    $Query.=llipn_quote_smart($itemqtysordered).",";
    $Query.=llipn_quote_smart($itempricesordered).",";
    if ($for_auction=="true")
      $Query.=llipn_quote_smart("Y").",";
    else 
      $Query.=llipn_quote_smart("N").",";
    $Query.=llipn_quote_smart($auction_buyer_id).",";
    $Query.=llipn_quote_smart($auction_closing_date).",";
    $Query.=llipn_quote_smart("OK");
    $Query.=")";
    $mysql_result = llipn_mysql_query($Query, $mysql_linklok);
	
	// POPULATE USERS TABLE
    $Query = "INSERT INTO " . $DbTableNameUsers . " (";
    $Query.="orderno,";
    $Query.="datetime,";
    $Query.="name,";
    $Query.="email,";
    ...
    $Query.=llipn_quote_smart($invoice).","; //$Query.=llipn_quote_smart($txn_id).",";
    $Query.=llipn_quote_smart(date("Y-n-d H:i:s",time())).",";     
    $Query.=llipn_quote_smart($first_name." ".$last_name).",";
    $Query.=llipn_quote_smart($payer_email).",";
    $Query.=llipn_quote_smart($txn_id).",";   //$Query.=llipn_quote_smart($invoice).","; 
    if ($num_cart_items>0)
      $Query.=$num_cart_items.",";
    else  
      $Query.="1,";
    ...
    // Now update ip address which may only be known if called as return
    if ($calledasreturn)
    {
      $clip=$clientipaddr;
      if ($clientipaddr=="0.0.0.0")
        $clip=trim(strtok($_SERVER['REMOTE_ADDR'],","));
  ...
  if ($payment_status == "Refunded")
  {
    ...
    $Query = "UPDATE " . $DbTableNameUsers . " SET orderstatus='Refunded' WHERE orderno='" . $parent_txn_id . "'";
    $mysql_result = llipn_mysql_query($Query, $mysql_linklok);	
  }
  ...
  if (($payment_status == "Completed") && ($txn_type == "reversal"))
  {
    ...
    $Query = "UPDATE " . $DbTableNameUsers . " SET orderstatus='Reversed' WHERE orderno='" . $parent_txn_id . "'";
    $mysql_result = llipn_mysql_query($Query, $mysql_linklok);	
  }
  ...
  if (($payment_status == "Cancelled") && ($txn_type == "reversal"))
  {
    ...
    $Query = "UPDATE " . $DbTableNameUsers . " SET orderstatus='OK' WHERE orderno='" . $parent_txn_id . "'";
    $mysql_result = llipn_mysql_query($Query, $mysql_linklok);	
  }
}
...
function storeorderllprodsmysql($ProdId, $ProdDesc)
{
  ...
  $Query = "UPDATE " . $DbTableNameOrders . " SET llnumitems=".count($ProdId).", llitemnumber=".llipn_quote_smart($llitemnumber).", llitemname=".llipn_quote_smart($llitemname)." WHERE orderno=" . llipn_quote_smart($txn_id);
  $mysql_result = llipn_mysql_query($Query, $mysql_linklok); 
}
...
function get_s3_url($location,$expires,$operation="GET")
{
  ...
  $extension=fileextension($resource);  
  ...
  $signature = urlencode(hex2b64(hmacsha1($secretaccesskey,$string_to_sign)));
...
function filesize_s3($location)
{
  ...
  $url=get_s3_url($location,time()+$ServerTimeAdjust,"HEAD");
...
error_log("\n".'IPN: End of file', 3, dirname(__FILE__)."/../../error_log");
