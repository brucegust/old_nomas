...
if(isset($sid) && $sid > '') 
{
   ...
   if(!$link = db_connect_site()) {
	  $errorMsg = "System temporarily unavailable. Please try again later."; 
	   ...
	   $the_sid=trim($sid);
	  if(!$result = mysqli_query($link,"SELECT * FROM $ceuusersTBL WHERE sid='$the_sid' LIMIT 1")) {  
	     $errorMsg = "Database problem. Please try again later.";
	  } else {
		  //echo "SELECT * FROM $ceuusersTBL WHERE sid='$the_sid' LIMIT 1";
...
function showLinks($kind, $orderno) 
{
   ...
   if(!$link = db_connect_site()) {
	  ...
	  if($_GET['ordno']==1546389225)
	  {
		 ...
		 $sql="SELECT id,description,location,vidlocation FROM ceu_products WHERE id IN ('v01','v02','v03','v04','v05','v06','v07','v08','v09','v10','v11')";
	  }
		elseif($_GET['ordno']==1545903710)
		{
			$sql="SELECT id,description,location,vidlocation FROM ceu_products WHERE id IN  ('v06');";
		}
