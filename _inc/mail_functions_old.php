...
global $dloadPass;
   ...
   if(!isset($hash['dloadCode']) || empty($hash['dloadCode'])) {
	 return;
	 exit;
   }      
   ...
   $hashPword = preg_replace ('/[^a-z0-9]/i', '', $hash['dloadCode']);
   $hashPword = strtolower($hash['dloadCode']);
...
function checkSymposium2013($hash) {
global $formError, $toEmail, $Subject, $symposium2013TBL;
   ...
   // $securimage = new Securimage();
	...
	if(isset($form_vals["nomas_num"]) && !empty($form_vals["nomas_num"])) {		
...
/*	
    ...
    $formError['captcha_code'] = '';
    if ($securimage->check($form_vals['captcha_code']) == false || $securimage->check($form_vals['captcha_code']) == ''){
		
	   //$code = $securimage->check($form_vals['captcha_code']);
	   echo "CODE form_vals: " . $form_vals['captcha_code'] . "<br>";
	   echo ($securimage->check($form_vals['captcha_code']) == true ) ? "FALSE" : "TRUE";
	   //echo ($code = false) ? "FALSE" : "TRUE";
	   exit;		
...
*/
    ...
    $link = db_connect_site();
    if($result = mysqli_query($link,"INSERT INTO $symposium2013TBL (id,date_in,check_amt,name,profession,employer,city_state,phone1,email,home_addr,nomas_num, nurses_num,
                                                            s1_a,s1_b,s1_c,s2_a,s2_b,s2_c,s3_a,s3_b,s3_c,s4_a,s4_b,s4_c,message) 
															
                                VALUES (NULL,'$date_in','$check_amt','$name','$profession','$employer','$city_state','$phone1','$email','$home_addr','$nomas_num','$nurses_num',
								        '$s1_a','$s1_b','$s1_c','$s2_a','$s2_b','$s2_c','$s3_a','$s3_b','$s3_c','$s4_a','$s4_b','$s4_c','$message')")) { 
    }
