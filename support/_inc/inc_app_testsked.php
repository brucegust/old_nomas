...
global $testskedTBL, $min_test_num, $max_test_num;	   
   ...
   if(is_array($post) && count($post)) {
	  ...
	  if(!preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $form_vals['entered'])){
		 $errorMsg[] = 'Date entered must be formatted as yyyy-mm-dd.';
	  }	  	  
	  ...
	  // test number
	  if( empty($form_vals['testnum']) || $form_vals['testnum'] < $min_test_num || $form_vals['testnum'] > $max_test_num ) {
	  ...
	  if(!preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $form_vals['weekopen'])){
		 $errorMsg[] = 'Week open date must be formatted as yyyy-mm-dd.';		 
	  }
...
global $testskedTBL, $min_test_num, $max_test_num, $rlli_numbers, $today;	   
   ...
   if(is_array($post) && count($post) > 0) {
	  ...
	  if(!preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $form_vals['weekclose'])){
		 $errorMsg[] = 'Week Close date must be formatted as yyyy-mm-dd.';		 
	  }	  
	  ...
	  if(isset($form_vals['winopen']) && !empty($form_vals['winopen']) || $form_vals['winopen'] <> '0000-00-00') {
		 if(!preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $form_vals['winopen'])){
		    $errorMsg[] = 'Window Open date must be formatted as yyyy-mm-dd.';			
		 }	  	  
      } 
	  ...
	  if(isset($form_vals['winclose']) && !empty($form_vals['winclose']) || $form_vals['winclose'] <> '0000-00-00') {
		 if(!preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $form_vals['winclose'])){
			$errorMsg[] = 'Window Close date must be formatted as yyyy-mm-dd.';			
		 }	  	  
      }	  	  	  
...
global $rlli_array;
   ...
   if(in_array($sid,$rlli_array)) {	   
      ...
      switch ($sid) {
         case 2:
            return " Reliability Test #2";
			break;
		 case 3:
			return " Reliability Test #3";
			break;
		 case 4:
			return " Re-Licensing Test #1";
			break;
		 case 5:
			return " Re-Licensing Test #2";
			break;			
      }		
